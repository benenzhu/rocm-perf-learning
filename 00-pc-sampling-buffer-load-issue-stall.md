...
### 4.4 用 PC offset 区分主循环和 prologue/epilogue

到这里为止都是全局统计。但有个问题没回答:**这些停顿是发生在主循环里(可优化),还是 prologue/epilogue 里(一次性开销,再怎么改也消不掉)?**

官方 analyze 的输出里有 **`offset` 列**(每个 PC 的代码地址),拿它就能分段。方法很简单——**主循环体被重复执行,采样 count 会高出一个数量级**:

| 区间 | PC 数 | 平均 count | `buffer_load_dwordx4` | `buffer_store_short` |
|---|---|---|---|---|
| prologue `0x0000–0x1e00` | 135 | 2.2 | 0 | 0 |
...
```
                     count  stalled   ARB_WIN_EX  ALU_DEP
主循环内               2815     1817        1277      502
主循环外                  0        0           0        0
```

**64 条 load 的 PC 全部落在主循环内,一条都不在 prologue。** 所以那 502 个 `ALU_DEP` 是**实打实可优化的**,不是启动开销。

> **这个分段值得做进你自己的流程里。** 全局统计会把"必须付出的一次性成本"和"真正的浪费"混在一起;
> 按 offset 分段之后,才知道优化的天花板在哪。

### 4.5 一个具体发现:循环不变的地址计算

`buffer_load_dwordx4` 有 **502 个 `ALU_DEP`(占它停顿的 27.6%)**。load 在等什么?
...
```asm
.LBB0_1:
    v_add_u32_e32 v66,  v181, v183     ← 每轮重算基址
    v_add_u32_e32 v67,  v181, v182
    v_add_u32_e32 v116, 0x2000, v66    ← 固定偏移
    ...
    buffer_load_dword v247, v66,  s[4:7], 0 offen offset:256    ← 紧接着就用
    buffer_load_dword v248, v116, s[4:7], 0 offen offset:256
    ...
```

**这 10 条地址加法,每轮循环都重算一遍,算完立刻被 load 用掉——RAW 依赖直接暴露。**

**关键在于:它们其实是循环不变的。** 查 `v181/v182/v183` 在循环体内被写的次数:

```
v181  0 次   ← 完全不变(prologue 算好)
v182  1 次   ← 循环末尾 v_add_u32_e32 v182, 0x200, v182
v183  1 次   ← 循环末尾 v_add_u32_e32 v183, 0x200, v183
```

也就是说,**整个 swizzle 地址模式在 prologue 就固定了**,循环里唯一的变化是末尾那两条 `+0x200` 的步进(K 方向前进一个 tile)。而 `0x2000 / 0x8000 / 0xa000` 这些偏移**是编译期常量**。

**优化方向:把地址计算提到循环外。**

- 基址 `v66/v67` 及其派生的 `v116/v117/v242/v244/v245/v246` 在 prologue 算好
- 循环里只保留末尾那两条指针步进
- 或者更进一步,用 `buffer_load` 的 **`soffset`/立即数 offset 字段**承载那些常量偏移,连 VGPR 加法都省掉

**预期收益**:主循环 299 条 VALU 里去掉 10 条(3.3%),更重要的是**消除 load 前的 RAW 依赖链**——那 502 个 `ALU_DEP` 占主循环全部停顿的 5.3%。

> ⚠️ 但要清醒:同一批 load 的 `ARBITER_WIN_EX_STALL` 是 **1277**,是 `ALU_DEP` 的 **2.5 倍**。
> 地址计算是个干净、确定的收益,但**发射受限才是主要矛盾**。先做哪个取决于改动成本。
