...
这也解释了为什么**提高 L2 命中率对我们这个问题毫无帮助**:TA 在 ③ 就放行了,它根本不知道后面命中没命中。
...
t2  TA 算地址+合并   平均 12.9 周期(取决于跨几个 64B 区间)
    ═══════════ 指令让出发射流水线,后续指令可进 ═══════════
t3  TCP 查 tag      命中即返回,未命中下发 L2
t4  TCC (L2)
t5  EA → HBM        t3~t5 合计几百周期
t6  数据写进 VGPR/LDS,vmcnt 递减
                    🔴 卡这里 = s_waitcnt vmcnt 等待

    t0~t2 决定发射吞吐  |  t3~t6 决定延迟
```

**这个区分直接决定该做什么:**

| 对发射受限**没用**(作用在 t3~t6) | 对发射受限**有用**(作用在 t2) |
|---|---|
| 提高 L2 / vL1D 命中率 | 减少 VMEM 指令条数 |
